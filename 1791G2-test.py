...
here = Path(__file__)
FILEPATH = Path(here.parent, "1791G2.py").__str__()
...
def simulate(t=1000):
    ...
    for n in range(2, 5):
        for c in range(1, 11):
            for line in product(range(1, n+1), repeat=n):
                string += f"{n} {c}".encode() + b"\n"
                string += " ".join([str(_) for _ in line]).encode() + b"\n"
                inputs.append((a, b))
